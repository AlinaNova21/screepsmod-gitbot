...
function deploy(req, repo, branch) {
  const { user, body } = req
  console.log('Deploy', user.username, repo)
  return request.get(`https://api.github.com/repos/${repo}/contents`)
    .then(contents => {
    ...
    .catch(err => {
      console.error('Gitbot',user.username, repo, branch, err)
    })
}

function getCode(repo, directory) {
  request.get(`https://api.github.com/repos/contents/${directory}`)
